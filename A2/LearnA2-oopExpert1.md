...
public class Singleton {//饿汉式单例类：拿对象的时候对象早已生成，无论是用不用，都会创建。
    private Singleton(){}//1、私有化构造器 除了内部 其他地方不能创建对象
...
1、枚举类的写法：**修饰符 enum 枚举类名{名称1，名称2，名称3; 其他成员...}**
...
public abstract class A {//抽象的本质是不能有对象，抽象类的基本作用就是被继承
    ...
    public A(){System.out.println("抽象类A的构造方法");}//抽象类有构造方法 必须先执行父类构造 → 子类构造 如果父类没有无参构造方法，编译器就无法自动插入 super()，这时候必须显式写出调用哪个父类构造器。不然报错
...
（3）、一类继承父类，同时又实现接口，若父类与接口中有同名的方法，则会优先实现父类的方法。（若想调用接口的我们只能新建中转方法，接口名.super.方法名）
